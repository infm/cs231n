...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    a1, _ = affine_forward(X, self.params['W1'], self.params['b1'])
    r1, _ = relu_forward(a1)
    scores, _ = affine_forward(r1, self.params['W2'], self.params['b2'])
    ...
    da1, grads['W2'], grads['b2'] = affine_backward(dscores, (a1, W2, b2))
    grads['W2'] += self.reg * W2
    _, grads['W1'], grads['b1'] = affine_backward(da1, (X, W1, b1))
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in xrange(1, len(dims)):
      ...
      current_dim = dims[i]
      prev_dim = dims[i - 1]
      self.params['W' + stri] = weight_scale * np.random.randn(
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(1, self.num_layers):
      ...
      bi = self.params['b' + stri]
      if self.use_batchnorm:
          inp, cache = affine_bn_relu_forward(inp, Wi, bi,
              self.params['gamma' + stri],
              self.params['beta' + stri],
              self.bn_params[i - 1])
      else:
        inp, cache = affine_relu_forward(inp, Wi, bi) 
      if self.use_dropout:
        inp, dropout_cache = dropout_forward(inp, self.dropout_param)
        cache = cache + (dropout_cache, )
      caches.append(cache)
    strn = str(self.num_layers)
    scores, cache = affine_forward(inp, self.params['W' + strn],
            self.params['b' + strn])
    caches.append(cache)
    ...
    for i in range(self.num_layers - 2, -1, -1):
      ...
      cache = caches[i]
      if self.use_dropout:
        # Crappy implementation, sorry about this
        dropout_cache = None
        if self.use_batchnorm:
          rc_cache, bn_cache, relu_cache, dropout_cache = cache
          cache = (rc_cache, bn_cache, relu_cache)
        else:
          rc_cache, relu_cache, dropout_cache = cache
          cache = (rc_cache, relu_cache)
        dres = dropout_backward(dres, dropout_cache)
      if self.use_batchnorm:
        dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dres, cache)
        grads['gamma' + stri] = dgamma
        grads['beta' + stri] = dbeta
      else:
        dx, dw, db = affine_relu_backward(dres, cache)
      grads['W' + stri] = dw
      grads['b' + stri] = db
